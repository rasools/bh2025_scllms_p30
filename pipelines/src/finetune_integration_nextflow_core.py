...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fine-tune scGPT on a single-cell dataset."
    )
    parser.add_argument("--dataset_name", type=str, default="PBMC_10K", help="Dataset name")
    parser.add_argument("--data_is_raw", type=lambda x: str(x).lower() == "true", default=True)
    parser.add_argument("--adata_path", type=str, default=None, help="Path to dataset (AnnData or folder)")
    parser.add_argument("--load_model", type=str, default=None, help="Path to pretrained model directory")
    parser.add_argument("--output_dir", type=str, default="./output", help="Directory to save results")
    parser.add_argument("--vocab_path", type=str, default=None, help="Path to vocab.json if needed")
    parser.add_argument("--ori_batch_col", type=str, default="batch", help="Label of the original batch column")
    ...
    parser.add_argument("--pre_norm", type=lambda x: str(x).lower() == "true", default=False)
...
print(f"Loading data from --adata_path: {args.adata_path}")

# Check if the H5AD file exists
if not os.path.isfile(args.adata_path):
    raise FileNotFoundError(f"The --adata_path file was not found: {args.adata_path}")

print("Loading .h5ad file...")
adata = sc.read_h5ad(args.adata_path)

data_is_raw = args.data_is_raw
print(f"Setting data_is_raw to: {data_is_raw}")

# --- Inspect adata.var ---
print(adata.var)
print("\n--- Gene columns (adata.var) ---")
print(list(adata.var.columns))
print(f"\nCurrent .var index is: {adata.var.index.name}")
adata.obs["celltype"] = adata.obs["str_labels"].astype("category")

# make the batch category column
adata.obs["str_batch"] = adata.obs[args.ori_batch_col].astype(str)
