...
adata_embedded.obs["celltype"] = adata_embedded.obs[celltype_key].astype("category")

if batch_key not in adata.obs.columns:
    adata.obs[batch_key] = "0"
if batch_key not in adata_embedded.obs.columns:
    adata_embedded.obs[batch_key] = "1"

adata.obs["batch_id"] = adata.obs[batch_key]
adata_embedded.obs["batch_id"] = adata_embedded.obs[batch_key]
...
if (gene_name_key in adata_embedded.var.columns) and (adata_embedded.var.index.name != gene_name_key):
    adata_embedded.var.set_index(adata_embedded.var[gene_name_key].astype(str), inplace=True)

adata.var["gene_name"] = adata.var.index.astype(str).tolist()
adata_embedded.var["gene_name"] = adata_embedded.var.index.astype(str).tolist()

data_is_raw = False
filter_gene_by_counts = False
adata_embedded_raw = adata_embedded.copy()

# Concatenate (original logic)
adata = adata.concatenate(adata_embedded, batch_key="batch_id")
...
adata_embedded = adata[adata.obs["batch_id"] == 1].copy()
adata = adata[adata.obs["batch_id"] == 0].copy()

preprocessor(adata, batch_key=None)
preprocessor(adata_embedded, batch_key=None)
...
def model_embed(
    ...
    output = {}
    cell_emb = model._get_cell_emb_from_layer(gene_emb, values)
    output["emb"] = gene_emb
...
# %%
def evaluate_embedding(model: nn.Module, loader: DataLoader, return_raw: bool = False, epoch=None, normalise=False):
    model.eval()
    # total_loss = 0.0
    # total_error = 0.0
    # total_dab = 0.0
    ...
    # embeddings = np.zeros(
    #     (len(dataset), embsize), dtype=np.float32
    # )
    with torch.no_grad():
        count = 0
        for batch_data in loader:
    ...
    return None, cell_embeddings


def embed(model: nn.Module, adata: DataLoader) -> float:
    all_counts = (
...
# %% [markdown]
# ## Step 5: Inference with scGPT model
# In the cell-type annotation task, the scGPT predicts cell-type labels for query set as inference. The model performance is evaluated on standard classificaton metrics. Here we visualize the predicted labels over the scGPT cell embeddings, and present the confusion matrix for detailed classification performance on the cell-group level.

# %%
# adata_embedded_raw = None
embeddings, cell_embeddings = embed(model, adata_embedded)
adata_embedded_raw.obsm["scGPT"] = cell_embeddings

# %%
# Save immediately after embedding (critical output)
output_h5ad_path = f"{output_dir}/embeddings.h5ad"
adata_embedded_raw.write_h5ad(output_h5ad_path)
...
# Compute UMAP
compute_umap = True
umap_png_path = f"{output_dir}/umap.png"
adata_embedded = adata_embedded_raw.copy()
if compute_umap:
    import sys
    import numpy as np
